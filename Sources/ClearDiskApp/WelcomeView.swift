            ...
            VStack(alignment: .leading, spacing: 28) {
                ...
                HStack(alignment: .center, spacing: 30) {
                    VStack(alignment: .leading, spacing: 18) {
                        Text("Make room for\nwhat’s next.")
                            .font(.system(size: 38, weight: .semibold)).tracking(-0.6)
                        ...
                        Button { state.startScan(path: "/") } label: {
                            Label("Scan my disk", systemImage: "internaldrive")
                        }.buttonStyle(PrimaryButtonStyle())
                        Text("We’ll check disk access before starting.")
        ...
        Button(action: action) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(subtitle).font(.system(size: 14)).foregroundStyle(UI.textSecondary)
                }
                Spacer()
...
struct ScanningView: View {
        ...
        VStack(alignment: .leading, spacing: 26) {
            HStack {
                Label("Scan in progress", systemImage: "viewfinder")
                    .font(.system(size: 14, weight: .semibold)).foregroundStyle(UI.accentLight)
                Spacer()
            ...
            HStack(alignment: .top, spacing: 20) {
                VStack(alignment: .leading, spacing: 16) {
                    HStack {
                        Text("Storage explorer").font(.system(size: 14, weight: .semibold))
                        Spacer()
                        Text("Building your map").font(.system(size: 14)).foregroundStyle(UI.textSecondary)
                    }
                    StorageMapArtwork(active: true).frame(minHeight: 200, maxHeight: .infinity)
                    Text("Scan illustration. Your measured storage map appears when ready.")
                        .font(.system(size: 14)).foregroundStyle(UI.textSecondary).lineSpacing(3)
                ...
                VStack(alignment: .leading, spacing: 20) {
                    HStack(spacing: 10) {
                        ...
                        Text(state.scanStage).font(.system(size: 14, weight: .semibold))
                    }
                    Divider()
                    metric(fmtBytes(state.progressBytes), "space measured", icon: "internaldrive")
                    ...
                    Label("Nothing is changed or deleted.", systemImage: "checkmark.shield")
                        .font(.system(size: 14)).foregroundStyle(UI.safeText)
                }.padding(22).frame(width: 270, alignment: .leading).frame(maxHeight: .infinity).card(radius: 16)
            }.frame(maxHeight: 355)
            HStack(spacing: 0) {
    ...
    private func metric(_ value: String, _ label: String, icon: String) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Label(label, systemImage: icon).font(.system(size: 14)).foregroundStyle(UI.textSecondary)
        }
    }
    private func scanStep(_ number: String, _ title: String, active: Bool) -> some View {
        HStack(spacing: 8) {
            Text(number).font(.system(size: 13, weight: .medium))
